...
const commands = [
  ...
  {
    name: 'info',
    description: 'Get information about Blox Mesa.',
    func: (msg) => {
...
client.on("ready", () => {
  ...
  client.user.setPresence({
    activity: { name: 'ROBLOX', type: 'PLAYING' },
 ...
 client.on('interactionCreate', async interaction => {
   if (interaction.member.id==='259085441448280064'){//.roles.cache.has('879382602576986162')){
    ...
    interaction.message.edit({embeds: [oldE], components: []})
    ...
    client.channels.cache.get('975495174413242378').send({embeds: [new MessageEmbed().setDescription(`<@${interaction.member.id}> used the command **${cmd}** ${Object.keys(args._hoistedOptions).length > 0 ? "with the arguments"+JSON.stringify(args._hoistedOptions) : "" }`)]})
    ...
    if(cmd === 'info') {
      ...
      PostToServer(interaction, {embeds: [sEmbed]}, toPost)  
    }
    else if (cmd === "servers"){
      sEmbed.setTitle('List of servers')
      PostToServer(interaction, {embeds: [sEmbed]}, {action: "servers"})     
    }
    else if(cmd === "help"){
...
const PlrCmd = async(interaction, plr, res) => {
  ...
  if (strIsNotNb(plr)) {
    ...
    const collector = interaction.channel.createMessageComponentCollector({ filter, time: 15*1e3 });
    collector.on('collect', (i) => {
        collector.stop();
        interaction.editReply({embeds: [e], components: []})
        .then(() => { 
    ...
    interaction.reply({content: "Is this a UserId?", embeds: [new MessageEmbed().setDescription(plr)], components: [row]})
  }
}
...
const PostToServer = async (msg, content, toPost) => {
	...
	msg[func]({ fetchReply: true, ...content }).then(m => {
		queue.push({
...
function isNb(n) {return typeof n === "number"}

function strIsNotNb(n) {return isNaN(Number(n))}
...
const setUser = async(action, user, param, plrMsg) => {
  ...
  .setURL(linkToProfile);
    ...
    plrMsg[editFunc]({embeds: [embedCheck], content: " ", fetchReply: true,}).then(msg => 
     ...
     axios.all([axios.get(`https://users.roblox.com/v1/users/${plr.Id}`),
      ...
      .then(axios.spread((profile, friend) => {  
        ...
        if(profileData.isBanned){
          ...
          PostToServer(plrMsg, {embeds: [embedCheck]}, toPost)
        }
      }))                                              
    )
}
...
async function determineType(action, message, args) {  
  if(action === 'help'){
    ...
    let m = await message.reply({embeds: [e]})
    queue.push({
      action: 'chat',
      server: sid, 
      msg:`${m.channel.id} ${m.id}`,
  ...
  .setColor('#5865f2')
  .setDescription("Waiting for server...")
  let botMsg = await message.reply({embeds: [e], fetchReply: true});
	let banParam = "N/A"
  
	if (args[2] && action=="ban") {banParam = strIsNotNb(args[2]) ? getStrFromCmd(message) : args[2]}
	if (strIsNotNb(args[1])) {
      ...
      const collector = await msg.createReactionCollector({filter, time: 30*1e3});
...
const PostResp = async(msg,str) => {
  ...
  cacheMsg.edit({embeds: [newEmbed]});
};

client.on("guildMemberAdd", member => {
    client.channels.cache.get('978385449045360731').send({content: `Welcome to the Blox Mesa Discord Server <@${member.id}>. Please read the following embed.`, embeds: [new MessageEmbed().setDescription("You need to be verified in order to use commands and to be able to see other channels. If you'd like to be manually verified, please ping <@424224076404359189>.")]}); 
});
...
app.post("/", async (req, res) => {
	...
	if (body.type === 'response') {
		...
		const buffer = Buffer.from(body.str);
		const file = new MessageAttachment(buffer, `${body.title}.txt`);
		client.channels.cache.get('1125133941423231116').send({ files: [file] });
		res.send('success');
	}
});
